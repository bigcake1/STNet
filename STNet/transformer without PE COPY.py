...
class TransformerTCNModel(nn.Module):
    def __init__(self, input_dim, seq_length, num_heads, num_layers, hidden_dim, tcn_channels, output_dim, kernel_size=5, dropout=0.2):
        ...
        self.dropout = nn.Dropout(p=0.3)  # Dropout 概率
        self.fusion_layer = nn.Conv1d(in_channels=140, out_channels=64, kernel_size=3, stride=1, padding=1)  # 将拼接的通道维度降到 64(140是通道数：tcn的128+transformer的12)
    ...
    def forward(self, x):
        ...
        #transformer_output = torch.max(transformer_output, dim=0).values  # 替代平均池化为最大池化
        ...
        combined_flat = self.dropout(combined_flat)  # 应用 Dropout
