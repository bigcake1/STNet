...
# 使用 DataLoader 创建分批数据加载器
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)  # 打乱训练数据
test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)  # 不打乱测试数据
...
for epoch in range(num_epochs):
    ...
    epoch_train_loss = 0  # 用于记录每个 epoch 的训练损失

    for data, target in train_loader:  # 从 train_loader 中按批次加载数据
        # Zero the gradients
        optimizer.zero_grad()
    ...
    epoch_test_loss = 0
    with torch.no_grad():
        for data, target in test_loader:  # 按批次加载测试数据
            y_pred_test = model(data)
            test_loss = criterion(y_pred_test, target)
