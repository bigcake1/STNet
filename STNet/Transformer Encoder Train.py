...
class TransformerEncoderModel(nn.Module):
    def __init__(self, input_dim, seq_length, num_heads, num_layers, hidden_dim, output_dim):
        ...
        # Output layers
        self.fc1 = nn.Linear(hidden_dim * seq_length, 128)
        self.fc2 = nn.Linear(128, output_dim)

    def _generate_positional_encoding(self, seq_length, model_dim):
        """
        Generate positional encoding matrix dynamically based on seq_length and model_dim
        :param seq_length: Sequence length of the input
        :param model_dim: Dimension of the hidden layer
        :return: Positional encoding tensor of shape (1, seq_length, model_dim)
        """
    ...
    def forward(self, x):
        ...
        # 2. Generate and add positional encoding
        positional_encoding = self._generate_positional_encoding(seq_length, x.size(-1)).to(x.device)
        x = x + positional_encoding  # (batch_size, seq_length, hidden_dim)

        # 3. Swap dimensions to match Transformer requirements: (seq_length, batch_size, hidden_dim)
        x = x.permute(1, 0, 2)

        # 4. Transformer Encoder
        x = self.transformer_encoder(x)  # (seq_length, batch_size, hidden_dim)

        # 5. Restore dimensions: (batch_size, seq_length, hidden_dim)
        x = x.permute(1, 0, 2)

        # 6. Flatten and pass through fully connected layers
        x = x.reshape(x.size(0), -1)  # (batch_size, seq_length * hidden_dim)
...
class CNNModelWithTCN(nn.Module):
    def __init__(self):
        ...
        # Define TCN layers
        self.tcn = TemporalConvNet(num_inputs=12, num_channels=[32, 128], kernel_size=3, dropout=0.2)
...
# 初始化模型
#model = TransformerEncoderModel(input_dim, seq_length, num_heads, num_layers, hidden_dim, output_dim)

# 定义模型参数
input_dim = 12         # 输入特征维度 (传感器数量)
seq_length = 60        # 序列长度
num_heads = 4          # Transformer 注意力头数量
num_layers = 2         # Transformer 层数
hidden_dim = 64        # Transformer 隐藏层维度
tcn_channels = [32, 64, 128]  # TCN 通道列表
